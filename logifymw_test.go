...
func TestLogIt2(t *testing.T) {
	var buf bytes.Buffer
	log := log.New(&buf, "", log.Ldate|log.Ltime)
	//log.SetOutput(&buf)

	ts := httptest.NewServer(logifymw.LogIt2(log, testHandler()))
	defer ts.Close()
	...
	// We should get a string like this:
	// "2020/05/04 13:34:12 GET  /                                                  200 5 3.2µs\n"
	ok, err := regexp.MatchString(`^\d\d\d\d\/\d\d\/\d\d \d\d:\d\d:\d\d GET  \/\s+ 200 5 .*s\n$`, buf.String())
	if err != nil {
		t.Fatalf("TestLogIt: error matching regular expression: %v", err)
	}
	if !ok {
...
func TestLogItMoreMore2(t *testing.T) {
	var buf bytes.Buffer
	log.SetOutput(&buf)

	ts := httptest.NewServer(logifymw.LogItMoreMore2(testHandler()))
	defer ts.Close()
	...
	// We should get a string like this:
	// "2020/05/04 13:33:13 127.0.0.1:44780 HTTP/1.1 GET  /                                                  Go-http-client/1.1 200 5 6.5µs\n"
	ok, err := regexp.MatchString(`^\d\d\d\d\/\d\d\/\d\d \d\d:\d\d:\d\d 127.0.0.1:\d+ HTTP/1.1 GET  \/\s+ Go-http-client\/1.1 200 5 .*s\n$`, buf.String())
	if err != nil {
		t.Fatalf("TestLogItMoreMore: error matching regular expression: %v", err)
	}
	if !ok {
